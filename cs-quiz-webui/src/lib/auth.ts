...
export type AdminQuestion = PlayQuestion & {
...
const TOKEN_KEY = 'csquiz-token'
const USER_KEY = 'csquiz-user'
...
export function fetchQuizzes(): Promise<Quiz[]> {
  return apiFetch<Quiz[]>('/api/v1/quizzes')
}

export function fetchQuiz(slug: string): Promise<Quiz> {
  return apiFetch<Quiz>(`/api/v1/quizzes/${encodeURIComponent(slug)}`)
}

export function playQuiz(slug: string): Promise<{ quiz: Quiz; questions: PlayQuestion[] }> {
  return apiFetch<{ quiz: Quiz; questions: PlayQuestion[] }>(
    `/api/v1/quizzes/${encodeURIComponent(slug)}/play`,
    {},
    true,
  )
}
...
): Promise<{ id: string; quiz_id: string; quiz_slug: string; correct: number; total: number }> {
  return apiFetch(
    `/api/v1/quizzes/${encodeURIComponent(slug)}/submit`,
    { method: 'POST', body: JSON.stringify({ answers }) },
    true,
  )
}

export function adminListQuizzes(): Promise<Quiz[]> {
  return apiFetch<Quiz[]>('/api/v1/admin/quizzes', {}, true)
}
...
export function adminUpdateQuiz(
  slug: string,
  body: {
    name?: string
    slug?: string
  ...
  return apiFetch<Quiz>(
    `/api/v1/admin/quizzes/${encodeURIComponent(slug)}`,
    { method: 'PATCH', body: JSON.stringify(body) },
    true,
  )
}

export function adminDeleteQuiz(slug: string): Promise<{ status: string }> {
  return apiFetch<{ status: string }>(
    `/api/v1/admin/quizzes/${encodeURIComponent(slug)}`,
    { method: 'DELETE' },
    true,
...
export function adminListQuestions(slug: string): Promise<AdminQuestion[]> {
...
export function adminUpdateQuestion(
  slug: string,
  id: string,
  body: Partial<QuestionPayload>,
): Promise<AdminQuestion> {
  return apiFetch<AdminQuestion>(
    `/api/v1/admin/quizzes/${encodeURIComponent(slug)}/questions/${encodeURIComponent(id)}`,
    { method: 'PATCH', body: JSON.stringify(body) },
    true,
  )
}

export function adminDeleteQuestion(slug: string, id: string): Promise<{ status: string }> {
  return apiFetch<{ status: string }>(
    `/api/v1/admin/quizzes/${encodeURIComponent(slug)}/questions/${encodeURIComponent(id)}`,
    { method: 'DELETE' },
    true,
