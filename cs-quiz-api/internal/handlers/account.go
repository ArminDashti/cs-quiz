...
func (h *Handler) GetProfile(c *gin.Context) {
	...
	rows, err := h.db.QueryContext(c.Request.Context(), `
		SELECT s.id, s.user_id, s.quiz_id, q.slug, s.correct, s.total, s.created_at
		FROM scores s
	...
	scores := make([]gin.H, 0)
	for rows.Next() {
		var id, uid, quizID, quizSlug string
		var correct, total int
		var createdAt interface{}
		if err := rows.Scan(&id, &uid, &quizID, &quizSlug, &correct, &total, &createdAt); err != nil {
			writeError(c, http.StatusInternalServerError, "could not load scores")
			return
		}
